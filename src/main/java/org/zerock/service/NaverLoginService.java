...
public class NaverLoginService {
    ...
    public UserDTO getUserInfo(String accessToken) {
        ...
        try {
            ...
            UserDTO userInfo = parseNaverResponse(response);
    ...
    public UserDTO parseNaverResponse(String jsonResponse) {
        try {
    ...
    // 기존 사용자가 있는지 확인
    public boolean processNaverLogin(UserDTO loginfo) {
        try {
            if (loginfo.getId() == null) {
                throw new RuntimeException("ID or Email is null. ID: " + loginfo.getId());
            }

            UserDTO neverid = naverLoginMapper.selectNaverMemberById(loginfo.getId());
